...
const getAllDiscipline = async (req, res, next) => {
    try {
        ...
        const [records, total] = await Promise.all([
            Discipline.find(filter).select('student reportedBy reason severity date action actionNote actionBy actionDate').populate('student', 'name inviteCode mobile').populate('reportedBy', 'name').populate('actionBy', 'name').sort({ date: -1 }).skip(skip).limit(limit),
