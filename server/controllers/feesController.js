...
const getAllFees = async (req, res, next) => {
    try {
        ...
        const [fees, total] = await Promise.all([
            Fees.find(filter).select('studentId feeType amount status dueDate paymentDate').populate('studentId', 'name inviteCode mobile').sort({ dueDate: -1 }).skip(skip).limit(limit),
            Fees.countDocuments(filter),
        ]);
