...
exports.addStudentsToSession = async (req, res, next) => {
    try {
        ...
        // Enforce one-session-per-student: block if already in another session
        ...
        for (const sid of studentIds) {
            ...
            if (existing) {
                const student = await User.findById(sid).select('name');
                alreadyAssigned.push({ studentId: sid, studentName: student?.name || sid, currentClass: existing.name });
            }
        }
        if (alreadyAssigned.length > 0) {
            const names = alreadyAssigned.map(a => `${a.studentName} (already in ${a.currentClass})`).join(', ');
            return res.status(400).json({
                success: false,
                message: `Cannot add — these students are already assigned to a class: ${names}. Remove them from their current class first.`,
                alreadyAssigned,
            });
        }
